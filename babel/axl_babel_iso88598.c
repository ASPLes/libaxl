...
/**
 * @brief Creates the translation table for iso-8859-8 representation
 *  to move information from iso-8859-8 to utf-8 and viceversa.
 */
axlBabelTable * axl_babel_build_iso88598_table (void) {

	axlBabelTable * table;

	/* create the table to hold information translate iso-8859-8 encoding */
	...
	table[170].size      = 2;
	table[170].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[170].buffer[1] = (unsigned char) 151; /* 0x97 */
	...
	table[186].size      = 2;
	table[186].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[186].buffer[1] = (unsigned char) 183; /* 0xb7 */
	...
	table[191].size      = 1;
	table[191].buffer[0] = 0; /* unsupported translation */
	...
	table[193].size      = 1;
	table[193].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 194 */
	table[194].size      = 1;
	table[194].buffer[0] = 0; /* unsupported translation */
	...
	table[196].size      = 1;
	table[196].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 197 */
	table[197].size      = 1;
	table[197].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 198 */
	table[198].size      = 1;
	table[198].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 199 */
	table[199].size      = 1;
	table[199].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 200 */
	table[200].size      = 1;
	table[200].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 201 */
	table[201].size      = 1;
	table[201].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 202 */
	table[202].size      = 1;
	table[202].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 203 */
	table[203].size      = 1;
	table[203].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 204 */
	table[204].size      = 1;
	table[204].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 205 */
	table[205].size      = 1;
	table[205].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 206 */
	table[206].size      = 1;
	table[206].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 207 */
	table[207].size      = 1;
	table[207].buffer[0] = 0; /* unsupported translation */
	...
	table[209].size      = 1;
	table[209].buffer[0] = 0; /* unsupported translation */
	...
	table[211].size      = 1;
	table[211].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 212 */
	table[212].size      = 1;
	table[212].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 213 */
	table[213].size      = 1;
	table[213].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 214 */
	table[214].size      = 1;
	table[214].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 215 */
	table[215].size      = 1;
	table[215].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 216 */
	table[216].size      = 1;
	table[216].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 217 */
	table[217].size      = 1;
	table[217].buffer[0] = 0; /* unsupported translation */

	/* store item associated to code 218 */
	table[218].size      = 1;
	table[218].buffer[0] = 0; /* unsupported translation */
	...
	table[223].size      = 3;
	table[223].buffer[0] = (unsigned char) 226; /* 0xe2 */
	table[223].buffer[1] = (unsigned char) 128; /* 0x80 */
	table[223].buffer[2] = (unsigned char) 151; /* 0x97 */

	/* store item associated to code 224 */
	table[224].size      = 2;
	table[224].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[224].buffer[1] = (unsigned char) 144; /* 0x90 */

	/* store item associated to code 225 */
	table[225].size      = 2;
	table[225].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[225].buffer[1] = (unsigned char) 145; /* 0x91 */

	/* store item associated to code 226 */
	table[226].size      = 2;
	table[226].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[226].buffer[1] = (unsigned char) 146; /* 0x92 */

	/* store item associated to code 227 */
	table[227].size      = 2;
	table[227].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[227].buffer[1] = (unsigned char) 147; /* 0x93 */

	/* store item associated to code 228 */
	table[228].size      = 2;
	table[228].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[228].buffer[1] = (unsigned char) 148; /* 0x94 */

	/* store item associated to code 229 */
	table[229].size      = 2;
	table[229].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[229].buffer[1] = (unsigned char) 149; /* 0x95 */

	/* store item associated to code 230 */
	table[230].size      = 2;
	table[230].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[230].buffer[1] = (unsigned char) 150; /* 0x96 */

	/* store item associated to code 231 */
	table[231].size      = 2;
	table[231].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[231].buffer[1] = (unsigned char) 151; /* 0x97 */

	/* store item associated to code 232 */
	table[232].size      = 2;
	table[232].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[232].buffer[1] = (unsigned char) 152; /* 0x98 */

	/* store item associated to code 233 */
	table[233].size      = 2;
	table[233].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[233].buffer[1] = (unsigned char) 153; /* 0x99 */

	/* store item associated to code 234 */
	table[234].size      = 2;
	table[234].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[234].buffer[1] = (unsigned char) 154; /* 0x9a */

	/* store item associated to code 235 */
	table[235].size      = 2;
	table[235].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[235].buffer[1] = (unsigned char) 155; /* 0x9b */

	/* store item associated to code 236 */
	table[236].size      = 2;
	table[236].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[236].buffer[1] = (unsigned char) 156; /* 0x9c */

	/* store item associated to code 237 */
	table[237].size      = 2;
	table[237].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[237].buffer[1] = (unsigned char) 157; /* 0x9d */

	/* store item associated to code 238 */
	table[238].size      = 2;
	table[238].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[238].buffer[1] = (unsigned char) 158; /* 0x9e */

	/* store item associated to code 239 */
	table[239].size      = 2;
	table[239].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[239].buffer[1] = (unsigned char) 159; /* 0x9f */

	/* store item associated to code 240 */
	table[240].size      = 2;
	table[240].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[240].buffer[1] = (unsigned char) 160; /* 0xa0 */

	/* store item associated to code 241 */
	table[241].size      = 2;
	table[241].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[241].buffer[1] = (unsigned char) 161; /* 0xa1 */

	/* store item associated to code 242 */
	table[242].size      = 2;
	table[242].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[242].buffer[1] = (unsigned char) 162; /* 0xa2 */

	/* store item associated to code 243 */
	table[243].size      = 2;
	table[243].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[243].buffer[1] = (unsigned char) 163; /* 0xa3 */

	/* store item associated to code 244 */
	table[244].size      = 2;
	table[244].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[244].buffer[1] = (unsigned char) 164; /* 0xa4 */

	/* store item associated to code 245 */
	table[245].size      = 2;
	table[245].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[245].buffer[1] = (unsigned char) 165; /* 0xa5 */

	/* store item associated to code 246 */
	table[246].size      = 2;
	table[246].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[246].buffer[1] = (unsigned char) 166; /* 0xa6 */

	/* store item associated to code 247 */
	table[247].size      = 2;
	table[247].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[247].buffer[1] = (unsigned char) 167; /* 0xa7 */

	/* store item associated to code 248 */
	table[248].size      = 2;
	table[248].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[248].buffer[1] = (unsigned char) 168; /* 0xa8 */

	/* store item associated to code 249 */
	table[249].size      = 2;
	table[249].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[249].buffer[1] = (unsigned char) 169; /* 0xa9 */

	/* store item associated to code 250 */
	table[250].size      = 2;
	table[250].buffer[0] = (unsigned char) 215; /* 0xd7 */
	table[250].buffer[1] = (unsigned char) 170; /* 0xaa */
	...
	table[253].size      = 3;
	table[253].buffer[0] = (unsigned char) 226; /* 0xe2 */
	table[253].buffer[1] = (unsigned char) 128; /* 0x80 */
	table[253].buffer[2] = (unsigned char) 142; /* 0x8e */

	/* store item associated to code 254 */
	table[254].size      = 3;
	table[254].buffer[0] = (unsigned char) 226; /* 0xe2 */
	table[254].buffer[1] = (unsigned char) 128; /* 0x80 */
	table[254].buffer[2] = (unsigned char) 143; /* 0x8f */
...
} /* end axl_babel_build_iso88598_table */
