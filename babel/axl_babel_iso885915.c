...
/**
 * @brief Creates the translation table for iso-8859-15 representation
 *  to move information from iso-8859-15 to utf-8 and viceversa.
 */
axlBabelTable * axl_babel_build_iso885915_table (void) {

	axlBabelTable * table;

	/* create the table to hold information translate iso-8859-15 encoding */
	...
	table[161].size      = 2;
	table[161].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[161].buffer[1] = (unsigned char) 161; /* 0xa1 */

	/* store item associated to code 162 */
	table[162].size      = 2;
	table[162].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[162].buffer[1] = (unsigned char) 162; /* 0xa2 */
	...
	table[165].size      = 2;
	table[165].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[165].buffer[1] = (unsigned char) 165; /* 0xa5 */

	/* store item associated to code 166 */
	table[166].size      = 2;
	table[166].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[166].buffer[1] = (unsigned char) 160; /* 0xa0 */
	...
	table[168].size      = 2;
	table[168].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[168].buffer[1] = (unsigned char) 161; /* 0xa1 */
	...
	table[170].size      = 2;
	table[170].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[170].buffer[1] = (unsigned char) 170; /* 0xaa */
	...
	table[174].size      = 2;
	table[174].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[174].buffer[1] = (unsigned char) 174; /* 0xae */

	/* store item associated to code 175 */
	table[175].size      = 2;
	table[175].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[175].buffer[1] = (unsigned char) 175; /* 0xaf */
	...
	table[180].size      = 2;
	table[180].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[180].buffer[1] = (unsigned char) 189; /* 0xbd */

	/* store item associated to code 181 */
	table[181].size      = 2;
	table[181].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[181].buffer[1] = (unsigned char) 181; /* 0xb5 */

	/* store item associated to code 182 */
	table[182].size      = 2;
	table[182].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[182].buffer[1] = (unsigned char) 182; /* 0xb6 */
	...
	table[184].size      = 2;
	table[184].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[184].buffer[1] = (unsigned char) 190; /* 0xbe */

	/* store item associated to code 185 */
	table[185].size      = 2;
	table[185].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[185].buffer[1] = (unsigned char) 185; /* 0xb9 */

	/* store item associated to code 186 */
	table[186].size      = 2;
	table[186].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[186].buffer[1] = (unsigned char) 186; /* 0xba */
	...
	table[188].size      = 2;
	table[188].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[188].buffer[1] = (unsigned char) 146; /* 0x92 */

	/* store item associated to code 189 */
	table[189].size      = 2;
	table[189].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[189].buffer[1] = (unsigned char) 147; /* 0x93 */

	/* store item associated to code 190 */
	table[190].size      = 2;
	table[190].buffer[0] = (unsigned char) 197; /* 0xc5 */
	table[190].buffer[1] = (unsigned char) 184; /* 0xb8 */

	/* store item associated to code 191 */
	table[191].size      = 2;
	table[191].buffer[0] = (unsigned char) 194; /* 0xc2 */
	table[191].buffer[1] = (unsigned char) 191; /* 0xbf */

	/* store item associated to code 192 */
	table[192].size      = 2;
	table[192].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[192].buffer[1] = (unsigned char) 128; /* 0x80 */

	/* store item associated to code 193 */
	table[193].size      = 2;
	table[193].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[193].buffer[1] = (unsigned char) 129; /* 0x81 */

	/* store item associated to code 194 */
	table[194].size      = 2;
	table[194].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[194].buffer[1] = (unsigned char) 130; /* 0x82 */

	/* store item associated to code 195 */
	table[195].size      = 2;
	table[195].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[195].buffer[1] = (unsigned char) 131; /* 0x83 */

	/* store item associated to code 196 */
	table[196].size      = 2;
	table[196].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[196].buffer[1] = (unsigned char) 132; /* 0x84 */

	/* store item associated to code 197 */
	table[197].size      = 2;
	table[197].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[197].buffer[1] = (unsigned char) 133; /* 0x85 */

	/* store item associated to code 198 */
	table[198].size      = 2;
	table[198].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[198].buffer[1] = (unsigned char) 134; /* 0x86 */

	/* store item associated to code 199 */
	table[199].size      = 2;
	table[199].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[199].buffer[1] = (unsigned char) 135; /* 0x87 */

	/* store item associated to code 200 */
	table[200].size      = 2;
	table[200].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[200].buffer[1] = (unsigned char) 136; /* 0x88 */

	/* store item associated to code 201 */
	table[201].size      = 2;
	table[201].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[201].buffer[1] = (unsigned char) 137; /* 0x89 */

	/* store item associated to code 202 */
	table[202].size      = 2;
	table[202].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[202].buffer[1] = (unsigned char) 138; /* 0x8a */

	/* store item associated to code 203 */
	table[203].size      = 2;
	table[203].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[203].buffer[1] = (unsigned char) 139; /* 0x8b */

	/* store item associated to code 204 */
	table[204].size      = 2;
	table[204].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[204].buffer[1] = (unsigned char) 140; /* 0x8c */

	/* store item associated to code 205 */
	table[205].size      = 2;
	table[205].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[205].buffer[1] = (unsigned char) 141; /* 0x8d */

	/* store item associated to code 206 */
	table[206].size      = 2;
	table[206].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[206].buffer[1] = (unsigned char) 142; /* 0x8e */

	/* store item associated to code 207 */
	table[207].size      = 2;
	table[207].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[207].buffer[1] = (unsigned char) 143; /* 0x8f */

	/* store item associated to code 208 */
	table[208].size      = 2;
	table[208].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[208].buffer[1] = (unsigned char) 144; /* 0x90 */

	/* store item associated to code 209 */
	table[209].size      = 2;
	table[209].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[209].buffer[1] = (unsigned char) 145; /* 0x91 */

	/* store item associated to code 210 */
	table[210].size      = 2;
	table[210].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[210].buffer[1] = (unsigned char) 146; /* 0x92 */

	/* store item associated to code 211 */
	table[211].size      = 2;
	table[211].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[211].buffer[1] = (unsigned char) 147; /* 0x93 */

	/* store item associated to code 212 */
	table[212].size      = 2;
	table[212].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[212].buffer[1] = (unsigned char) 148; /* 0x94 */

	/* store item associated to code 213 */
	table[213].size      = 2;
	table[213].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[213].buffer[1] = (unsigned char) 149; /* 0x95 */

	/* store item associated to code 214 */
	table[214].size      = 2;
	table[214].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[214].buffer[1] = (unsigned char) 150; /* 0x96 */

	/* store item associated to code 215 */
	table[215].size      = 2;
	table[215].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[215].buffer[1] = (unsigned char) 151; /* 0x97 */

	/* store item associated to code 216 */
	table[216].size      = 2;
	table[216].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[216].buffer[1] = (unsigned char) 152; /* 0x98 */

	/* store item associated to code 217 */
	table[217].size      = 2;
	table[217].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[217].buffer[1] = (unsigned char) 153; /* 0x99 */

	/* store item associated to code 218 */
	table[218].size      = 2;
	table[218].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[218].buffer[1] = (unsigned char) 154; /* 0x9a */

	/* store item associated to code 219 */
	table[219].size      = 2;
	table[219].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[219].buffer[1] = (unsigned char) 155; /* 0x9b */

	/* store item associated to code 220 */
	table[220].size      = 2;
	table[220].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[220].buffer[1] = (unsigned char) 156; /* 0x9c */

	/* store item associated to code 221 */
	table[221].size      = 2;
	table[221].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[221].buffer[1] = (unsigned char) 157; /* 0x9d */

	/* store item associated to code 222 */
	table[222].size      = 2;
	table[222].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[222].buffer[1] = (unsigned char) 158; /* 0x9e */

	/* store item associated to code 223 */
	table[223].size      = 2;
	table[223].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[223].buffer[1] = (unsigned char) 159; /* 0x9f */

	/* store item associated to code 224 */
	table[224].size      = 2;
	table[224].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[224].buffer[1] = (unsigned char) 160; /* 0xa0 */

	/* store item associated to code 225 */
	table[225].size      = 2;
	table[225].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[225].buffer[1] = (unsigned char) 161; /* 0xa1 */

	/* store item associated to code 226 */
	table[226].size      = 2;
	table[226].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[226].buffer[1] = (unsigned char) 162; /* 0xa2 */

	/* store item associated to code 227 */
	table[227].size      = 2;
	table[227].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[227].buffer[1] = (unsigned char) 163; /* 0xa3 */

	/* store item associated to code 228 */
	table[228].size      = 2;
	table[228].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[228].buffer[1] = (unsigned char) 164; /* 0xa4 */

	/* store item associated to code 229 */
	table[229].size      = 2;
	table[229].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[229].buffer[1] = (unsigned char) 165; /* 0xa5 */

	/* store item associated to code 230 */
	table[230].size      = 2;
	table[230].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[230].buffer[1] = (unsigned char) 166; /* 0xa6 */

	/* store item associated to code 231 */
	table[231].size      = 2;
	table[231].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[231].buffer[1] = (unsigned char) 167; /* 0xa7 */

	/* store item associated to code 232 */
	table[232].size      = 2;
	table[232].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[232].buffer[1] = (unsigned char) 168; /* 0xa8 */

	/* store item associated to code 233 */
	table[233].size      = 2;
	table[233].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[233].buffer[1] = (unsigned char) 169; /* 0xa9 */

	/* store item associated to code 234 */
	table[234].size      = 2;
	table[234].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[234].buffer[1] = (unsigned char) 170; /* 0xaa */

	/* store item associated to code 235 */
	table[235].size      = 2;
	table[235].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[235].buffer[1] = (unsigned char) 171; /* 0xab */

	/* store item associated to code 236 */
	table[236].size      = 2;
	table[236].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[236].buffer[1] = (unsigned char) 172; /* 0xac */

	/* store item associated to code 237 */
	table[237].size      = 2;
	table[237].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[237].buffer[1] = (unsigned char) 173; /* 0xad */

	/* store item associated to code 238 */
	table[238].size      = 2;
	table[238].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[238].buffer[1] = (unsigned char) 174; /* 0xae */

	/* store item associated to code 239 */
	table[239].size      = 2;
	table[239].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[239].buffer[1] = (unsigned char) 175; /* 0xaf */

	/* store item associated to code 240 */
	table[240].size      = 2;
	table[240].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[240].buffer[1] = (unsigned char) 176; /* 0xb0 */

	/* store item associated to code 241 */
	table[241].size      = 2;
	table[241].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[241].buffer[1] = (unsigned char) 177; /* 0xb1 */

	/* store item associated to code 242 */
	table[242].size      = 2;
	table[242].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[242].buffer[1] = (unsigned char) 178; /* 0xb2 */

	/* store item associated to code 243 */
	table[243].size      = 2;
	table[243].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[243].buffer[1] = (unsigned char) 179; /* 0xb3 */

	/* store item associated to code 244 */
	table[244].size      = 2;
	table[244].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[244].buffer[1] = (unsigned char) 180; /* 0xb4 */

	/* store item associated to code 245 */
	table[245].size      = 2;
	table[245].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[245].buffer[1] = (unsigned char) 181; /* 0xb5 */

	/* store item associated to code 246 */
	table[246].size      = 2;
	table[246].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[246].buffer[1] = (unsigned char) 182; /* 0xb6 */

	/* store item associated to code 247 */
	table[247].size      = 2;
	table[247].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[247].buffer[1] = (unsigned char) 183; /* 0xb7 */

	/* store item associated to code 248 */
	table[248].size      = 2;
	table[248].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[248].buffer[1] = (unsigned char) 184; /* 0xb8 */

	/* store item associated to code 249 */
	table[249].size      = 2;
	table[249].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[249].buffer[1] = (unsigned char) 185; /* 0xb9 */

	/* store item associated to code 250 */
	table[250].size      = 2;
	table[250].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[250].buffer[1] = (unsigned char) 186; /* 0xba */

	/* store item associated to code 251 */
	table[251].size      = 2;
	table[251].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[251].buffer[1] = (unsigned char) 187; /* 0xbb */

	/* store item associated to code 252 */
	table[252].size      = 2;
	table[252].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[252].buffer[1] = (unsigned char) 188; /* 0xbc */

	/* store item associated to code 253 */
	table[253].size      = 2;
	table[253].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[253].buffer[1] = (unsigned char) 189; /* 0xbd */

	/* store item associated to code 254 */
	table[254].size      = 2;
	table[254].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[254].buffer[1] = (unsigned char) 190; /* 0xbe */

	/* store item associated to code 255 */
	table[255].size      = 2;
	table[255].buffer[0] = (unsigned char) 195; /* 0xc3 */
	table[255].buffer[1] = (unsigned char) 191; /* 0xbf */

	/* return table created */
	return table;

} /* end axl_babel_build_iso885915_table */
